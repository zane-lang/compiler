...
void zane_main(void) {
	...
	char *a = zane_slot(outer, 1, 1);
	int64_t *b = zane_slot(outer, 8, 8);
	...
	char *c = zane_slot(inner, 16, 8);
	zane_scope_drain(inner);
	inner = zane_scope_enter();
	check(zane_slot(inner, 16, 8) == c);
	zane_scope_drain(inner);
	...
	*b = 42;
	inner = zane_scope_enter();
	memset(zane_slot(inner, 1024, 8), 0xff, 1024);
	zane_scope_drain(inner);
	check(*b == 42);
