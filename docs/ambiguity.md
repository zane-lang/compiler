...
- `just ambiguities` — bounded, parallel GLR search for complete ambiguous
  ...
  evidence only. `--max-frontiers` bounds resident memory per worker (an
  evicting dedup cache plus a cap on queued frontiers, roughly 2 KB per
  unit); the search itself is bounded by `--max-tokens` and `--timeout`,
  and a run that had to drop part of the space reports itself as
  interrupted. Witnesses are grouped by the conflict states they
